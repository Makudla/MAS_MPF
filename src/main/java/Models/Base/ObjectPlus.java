...
public abstract class ObjectPlus implements Serializable {
    ...
    public static void deleteObject(ObjectPlus obj) throws Exception {
        Class<?> theClass = obj.getClass();

        if(allExtents.containsKey(theClass)) {
            List<ObjectPlus> extent = allExtents.get(theClass);
            if (extent.contains(obj)) {
                extent.remove(obj);
            } else {
                throw new Exception("The object is not in the extent of the class: " + theClass.getSimpleName());
            }
        } else {
            throw new Exception("Unknown class: " + theClass.getSimpleName());
        }
    }
}
