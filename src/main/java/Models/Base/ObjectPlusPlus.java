...
/**
 * The class which simplifies connections between objects. Because it inherits from the Models.Base.ObjectPlus, it also helps with extent management.
 *
...
public abstract class ObjectPlusPlus extends ObjectPlus implements Serializable {
    ...
    /**
     * Deletes a link between objects for the given role and qualifier (if provided).
     * @param roleName
     * @param qualifier
     * @throws Exception
     */
    public void deleteLink(String roleName, Object qualifier) throws Exception {
        if (!links.containsKey(roleName)) {
            throw new Exception("No links for the role: " + roleName);
        }
        ...
        if (qualifier != null && objectLinks.containsKey(qualifier)) {
            ObjectPlusPlus targetObject = objectLinks.get(qualifier);

            // Remove the link
            objectLinks.remove(qualifier);

            // Remove reverse link
            targetObject.removeReverseLink(roleName, this);
        } else {
    ...
    /**
     * Removes reverse link between objects for the given roles.
     * @param roleName
     * @param targetObject
     */
    private void removeReverseLink(String roleName, ObjectPlusPlus targetObject) throws Exception {
        if (!links.containsKey(roleName)) {
            return;
        }
        ...
        for (Map.Entry<Object, ObjectPlusPlus> entry : objectLinks.entrySet()) {
            if (entry.getValue().equals(targetObject)) {
                objectLinks.remove(entry.getKey());
                return;
            }
        }
        ObjectPlus.deleteObject(this);
    }
